...
func (vs Versionstamp) String() string {
	return fmt.Sprintf("Versionstamp(%s, %d)", ByteSliceString(vs.TransactionVersion[:]), vs.UserVersion)
}
...
func (p *packer) encodeTuple(t TupleElementer, nested bool, versionstamps bool) {
	...
	for i, ee := range t.TupleElements() {
		...
		default:
			panic(fmt.Sprintf("unencodable element at index %d (%v, type %T)", i, ee, ee))
		}
	}
...
// UnpackTuple returns the tuple encoded by the provided byte slice, or an error if
