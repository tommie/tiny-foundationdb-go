...
func (vs Versionstamp) String() string {
	return fmt.Sprintf("Versionstamp(%s, %d)", internal.ByteSliceString(vs.TransactionVersion[:]), vs.UserVersion)
}
...
func (t Tuple) PackWithVersionstamp(prefix []byte) ([]byte, error) {
	...
	apiVersion, err := tinyfdb.GetAPIVersion()
	if err != nil {
		return nil, err
	}
